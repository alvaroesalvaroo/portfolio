...
const container = document.querySelector('#project-details-section .project-slider');
...
function init() {
    ...
    document.container.appendChild( controlsDomElement );

    rendererCSS3D = new CSS3DRenderer();
    rendererCSS3D.setSize( sizes.width, sizes.height );
    document.container.appendChild( rendererCSS3D.domElement );
    ...
    rendererWebGL.setSize( sizes.width, sizes.height );
    rendererWebGL.toneMapping = THREE.NeutralToneMapping;
    rendererWebGL.setAnimationLoop( animate );
    document.container.appendChild( rendererWebGL.domElement );

    camera = new THREE.PerspectiveCamera( 50, sizes.width / sizes.height, 1, 10000 );
